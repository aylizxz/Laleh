...
function captcha_new(): array {
    if (($_GET['force'] ?? '') === 'math') {
        // force math captcha (fallback when reCAPTCHA is blocked)
        $_SESSION['captcha_mode'] = 'math';
    } elseif (defined('RECAPTCHA_SITE_KEY') && RECAPTCHA_SITE_KEY) {
...
function captcha_verify(): void {
    ...
    if ($mode !== 'math' && defined('RECAPTCHA_SECRET_KEY') && RECAPTCHA_SECRET_KEY) {
